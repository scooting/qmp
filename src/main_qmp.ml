...
let array_lookup arr ix = if ix < Array.length arr then Some arr.(ix) else None
...
let parse_cli_args args =
  let rec work acc i =
    let name = array_lookup args i in
    let path = array_lookup args (i + 1) in
    match (name, path) with
    | (Some name, Some path) -> work ((name, path) :: acc) (i + 2)
    | _ -> acc
  in
  let names_and_paths = List.rev (work [] 2) in
  let root_dir = args.(1) in
